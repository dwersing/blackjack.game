...
def load_images(card_images):
    ...
    # Retrieves card images for each suit
    for suit in suits:
        for card in range(1, 11):
            name = 'C:/Users/dwersing/Desktop/Python/cards_png (1)/cards/{}_{}.{}'.format(str(card), suit, extension)
            image = tkinter.PhotoImage(file=name)
            card_images.append((card, image))

        for card in faces:
            name = f'C:/Users/dwersing/Desktop/Python/cards_png (1)/cards/{str(card)}_{suit}.{extension}'
            image = tkinter.PhotoImage(file=name)
            card_images.append((10, image))


# Deal the next card and add image/label to it
...
def deal_dealer():
    ...
    if player_score > 21:
        result_text.set('Bust!  Dealer Wins!')
...
def new_game():
    ...
    # Embedded card frame to hold images
...
deck = list(cards)
shuffle()
# List to store player and dealer hands
dealer_hand = []
player_hand = []
